...
def parse_args():
    parser = argparse.ArgumentParser(description="Run OpenFace behavior-only BDI baseline training.")
    parser.add_argument(
        "--base-config",
...
def run_from_config(cfgs):
    mode = str(cfgs.MODE)
    if mode != "behavior_baseline":
        raise ValueError(f"MODE must be 'behavior_baseline' for behavior baseline training, got: {mode}")

    from src.trainers.behavior_baseline_runner import run_behavior_baseline

    print("[INFO] START RUNNING OPENFACE BEHAVIOR-ONLY BASELINE...")
    run_behavior_baseline(cfgs)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("\n[UNKNOWN ERROR] Behavior baseline training was interrupted by an unexpected error.")
        print(f"[DETAIL] {e}")
        traceback.print_exc()
        sys.exit(1)
