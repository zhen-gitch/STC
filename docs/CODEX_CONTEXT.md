...
Shortcut-Regularized MTL 最新路线：正式路线是 Stage A 收口诊断 + Stage B 双干预。Stage A 只保留 layer-wise identity probe 和 prediction error x identity similarity coupling，用于确认身份信息层级位置及其与预测错误/偏置的耦合。Stage B 的正式对照为 E0 RGB MTL-Lite baseline、E1 severity-balanced regression、E2 identity-adversarial MTL branch、E3 combined。severity-balanced regression 的目标是缓解 score-bin / severity-bin 不均衡，不是人为提高 prediction variance。identity-adversarial branch 必须使用 GRL 或等价对抗机制；普通 subject classification auxiliary head 会强化身份信息，不等于去身份化。动态特征、feature delta、AU/landmark/pose/gaze delta 和 static-dynamic fusion 暂列 Stage C 待考虑项，只在 Stage B 后仍存在明显 static appearance shortcut 时启动。输入级 identity_texture_suppressed / boundary smoothing 2x2 结果保留为机制证据，不再作为下一阶段主线继续扩展。


文档整理状态：当前路线已前置到 `DOCS_GUIDE.md`、`CURRENT_STATUS.md`、`TODO.md`、`RGB_OVERFITTING_AUDIT_PLAN.md` 和 `OVERFITTING_MECHANISM_ROADMAP.md`。后续判断下一步时，优先读取 `TODO.md` 的“当前立即执行任务（权威入口）”。旧 input artifact / boundary 2x2 / temporal sampling 段落是历史证据，不再覆盖 Shortcut-Regularized MTL 主线。
