...
16. **AU-T0b coordinate-contract audit**（代码已实现，等待 OpenFace 输出与人工 overlay）：服务器/Windows aligned JPG 完整性门禁已以 `493141/493141 EXACT_MATCH`、`EXACT_PASS` 通过。已冻结 `D:\Tools\OpenFace` 的 OpenFace 2.2.0 和关键二进制/模型哈希，并新增 `scripts/run_openface_aligned_landmarks.ps1`。下一步先跑两个视频 debug，要求行数/帧连续性/必要列通过且 `success>=0.995`；再在独立目录完成 300 视频全量生成。随后将该目录作为 `--aligned-openface-root` 运行 T0b。coordinate core/CLI 的 `auto` 仅按显式 affine、aligned JPG 重检测、caller-supplied canonical similarity 三种合法来源选择；禁止直接比例缩放。自动通过只标记 `REVIEW_REQUIRED`，人工 overlay 未通过前不得进入 T1。
