...
16. **AU-T0b coordinate-contract audit**（代码已实现，待真实数据运行与人工 overlay）：先运行服务器/本地 aligned JPG integrity manifest，确认全部训练可见图片解码、`112x112` 尺寸及 SHA-256 为 `EXACT_PASS`；再用冻结版本 OpenFace 在本地完整图片序列上生成 aligned-space landmark。coordinate core/CLI 的 `auto` 仅按显式 affine、aligned JPG 重检测、caller-supplied canonical similarity 三种合法来源选择；禁止直接比例缩放。自动通过只标记 `REVIEW_REQUIRED`，人工 overlay 未通过前不得进入 T1。
